...
try{
  ...
  await page.evaluate(()=>{const el=[...document.querySelectorAll('li,a,div,span')].find(d=>(d.innerText||'').trim()==='套系'&&d.getBoundingClientRect().width>40&&d.getBoundingClientRect().height>20);if(el)el.click();});
  await sleep(8000);
  console.log('URL:',page.url());
  await page.screenshot({path:'/tmp/picbling-packages.png'});
  const dom=await page.evaluate(()=>{
    ...
    for(const el of nodes){
      ...
      out.push({tag:el.tagName.toLowerCase(),text,cls:(el.className||'').toString().slice(0,80),rect:{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)},bg:cs.backgroundColor,font:cs.fontSize+'/'+cs.fontWeight});
    }
    return out;
  });
  fs.writeFileSync('/tmp/picbling-packages-dom.json',JSON.stringify(dom,null,1));
