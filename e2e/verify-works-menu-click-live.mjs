...
const OUT = '/tmp/works-menu-click-live';
...
(async () => {
  ...
  const addInfo = await page.evaluate(() => ({ url: location.href, body: document.body.innerText.slice(0, 300) }));
  console.log('addInfo:', addInfo);

  // 测试自定义排序
  console.log('Step 3: sort mode');
  await page.goto(BASE_URL + '/works', { waitUntil: 'networkidle2' });
  await delay(2000);
  await page.evaluate(() => {
    const btn = Array.from(document.querySelectorAll('button')).find((b) => b.textContent.trim() === '⋯');
