        ...
        try {
          ...
          for (const tp of tempFilePaths) {
            try {
              ...
              previews.push({ tempFilePath: tp, digest, size, sign, dup: existSet.has(sign), error: false });
            } catch (e) {
              // 读取失败 → 放行（防误拦）
              previews.push({ tempFilePath: tp, digest: '', size: 0, sign: '', dup: false, error: true });
            }
          }
          const toUpload = previews.filter((p) => !p.dup);
          resolve({ previews, toUpload, dupCount: previews.length - toUpload.length, total: previews.length });
        } catch (e) {
          reject(e);
        }
      },
