...
// 底部 Tab（工作台 / 自媒体 / + / 订单 / 消息）——「套系」已与首页共用抽屉菜单入口，不再占底部 Tab 位
const TABS = [
  { key: 'home', label: '工作台', icon: 'monitor', to: '/' },
  { key: 'site', label: '自媒体', icon: 'ai', to: '/media' },
...
export default function MobileShell() {
  ...
  const tabRoots = ['/', '/media', '/orders', '/m/msg'];
  ...
  const activeKey = (() => {
    ...
    if (location.pathname.startsWith('/media')) return 'site';
...
<Route path="/todo" element={<Todo />} />
      ...
      {/* 底栏仅在首页一级 Tab 页显示（工作台 / 微官网 / 作品 / 消息）；二级业务页只显示内容，不占底栏空间 */}
