...
// 移动端「消息列表」二级页 —— 从「消息」参考图首页点击咨询分类进入（/m/messages?type=xxx）
...
export default function MobileMessageList() {
  ...
  const load = useCallback(async (p = 1, append = false) => {
    ...
    try {
      const r = await http.get('/api/mobile/message/list', { params: { page: p, pageSize, read_status: readStatus, biz_type: bizType } });
  ...
  const markAllRead = async () => {
    ...
    setMarkingAll(true);
    try { await http.put('/api/mobile/message/read-all'); load(1); } catch {} finally { setMarkingAll(false); }
  };
  ...
  const onItemTouchStart = (e, id) => { const t = e.touches[0]; touchRef.current = { id, x: t.clientX, y: t.clientY }; };
  ...
  const bizLabel = BIZ_META[bizType] ? BIZ_META[bizType].label : '全部消息';
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#F5F5F7' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', height: 44, padding: '0 12px', background: '#fff', borderBottom: `1px solid ${DIV}` }}>
        <button onClick={() => nav(-1)} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', padding: 4, color: TEXT, fontSize: 20, display: 'flex', alignItems: 'center' }}>‹</button>
        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', fontSize: 17, color: TEXT }}>{bizLabel}</div>
            ...
            {list.map((m) => {
              ...
              return (
                <div key={m.id} style={{ position: 'relative', marginBottom: 10, overflow: 'hidden', borderRadius: 12 }}>
                  ...
                  <div
                    onClick={() => { if (swipedId === m.id) { setSwipedId(null); return; } nav('/m/messages/' + m.id); }}
