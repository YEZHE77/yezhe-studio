...
// 移动端「预约消息」二级页 —— 从消息页点击【预约消息】进入
// 仅展示预约消息（sub_type=reserve，与订单消息 OrderMessages 的排除规则互补）
// 规范：Glassmorphism 玻璃顶栏 + Soft-UI 卡片；禁止字体加粗，仅灰度/字号/间距分层
// 顶部：返回 + 标题 + ⋯ 菜单（批量标记已读 / 清空消息）
// 点击消息：标记已读 + 跳预约管理页 /reservations（预约消息 biz_id 存预约 id）
const TEXT = '#1f2329';
const SUB = '#6E6E73';
const FAINT = '#AEAEB2';
const DIV = '#EEF0F3';
...
export default function ReserveMessages() {
  ...
  // 仅展示预约消息：sub_type=reserve（与订单消息 OrderMessages 的 sub_type_not=reserve 互补）
  const query = { sub_type: 'reserve' };
  ...
  const clearAll = async () => {
    setMenuOpen(false);
    if (!window.confirm('确定清空预约消息？此操作不可恢复。')) return;
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'linear-gradient(180deg, #EEF1F5 0%, #F5F7FA 100%)' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(255,255,255,0.66)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.6)' }}>
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', height: 44, padding: '0 12px' }}>
          ...
          <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', fontSize: 17, color: TEXT }}>预约消息</div>
