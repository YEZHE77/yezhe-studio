  ...
  return (
    <div style={{ maxWidth: 1050 }}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div>
          <h1 className="text-xl text-fg">客户管理</h1>
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="搜索昵称 / 电话"
          className="border border-line rounded-lg px-3 py-2 text-sm bg-panel text-fg outline-none focus:border-brand w-full sm:w-56" />
      </div>

      <div className="bg-panel border border-line rounded-xl2 overflow-hidden">
        <div className="overflow-x-auto">
          <div className="grid grid-cols-12 min-w-[640px] px-4 py-3 text-xs text-muted border-b border-line">
          ...
          {filtered.filter(Boolean).map((c) => (
            <div key={c.openid} onClick={() => open(c.openid)}
              className="grid grid-cols-12 min-w-[640px] px-4 py-3 text-sm items-center border-b border-line last:border-0 hover:bg-panel2 cursor-pointer transition">
              ...
              <div className="col-span-2 text-right text-fg">¥{c.spent.toLocaleString()}</div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-end z-50" onClick={() => setDetail(null)}>
          <div className="w-full max-w-md bg-panel h-full overflow-auto p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg text-fg">客户详情</h2>
            ...
            {detail && !detail.error && (
              <div className="space-y-5">
                ...
                <div>
                  <div className="text-[15px] text-fg mb-2">订单记录（{detail.orders.length}）</div>
                  <div className="space-y-2">
                    {detail.orders.map((o) => (
                      <div key={o.id} className="border border-line rounded-lg p-3 text-sm">
                        <div className="flex justify-between">
                          <span className="text-fg">{o.packageName || '自定义订单'}</span>
                ...
                <div>
                  <div className="text-[15px] text-fg mb-2">预约记录（{detail.appointments.length}）</div>
                  <div className="space-y-2">
