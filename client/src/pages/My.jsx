    ...
    <div className="min-h-screen" style={{ background: PAGE_BG, color: '#2c2c2c' }}>
      ...
      <div className="flex items-center gap-4 px-6 pb-10 pt-4 text-white" style={{ background: '#1f1f1f' }}>
        ...
        <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full" style={{ background: 'rgba(255,255,255,0.16)' }}>
          {studio.logo ? (
            ...
            <span className="text-xl">{(studio.name || '')[0]}</span>
          )}
        </div>
        <div className="min-w-0 flex-1">
          <div className="truncate text-base">{customer ? (maskPhone(customer.phone) || '已登录') : (studio.name || '')}</div>
      ...
      <div className="px-4 pb-10 pt-6">
        <div className="rounded-2xl bg-white p-5">
          <div className="mb-3 text-sm" style={{ color: TEAL }}>关于{studio.name} ›</div>
          <div className="whitespace-pre-line text-xs leading-relaxed" style={{ color: '#666' }}>{studio.intro || '用影像记录时光。'}</div>
        </div>
      </div>
